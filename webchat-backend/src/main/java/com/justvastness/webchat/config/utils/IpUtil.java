...
public class IpUtil extends Ipv4Util {
    ...
    public static String getRemoteAddr(HttpServletRequest request) {
        ...
        if (log.isDebugEnabled()) {
            log.info("[IP X-Forwarded-For]={}", ip);
        }
        if (StringUtils.isEmpty(ip) || "unknown".equalsIgnoreCase(ip)) {
            ip = request.getHeader("Proxy-Client-IP");
            if (log.isDebugEnabled()) {
                log.info("[IP Proxy-Client-IP]={}", ip);
            }
        }
        if (StringUtils.isEmpty(ip) || "unknown".equalsIgnoreCase(ip)) {
            ip = request.getHeader("WL-Proxy-Client-IP");
            if (log.isDebugEnabled()) {
                log.info("[IP WL-Proxy-Client-IP]={}", ip);
            }
        }
        if (StringUtils.isEmpty(ip) || "unknown".equalsIgnoreCase(ip)) {
            ip = request.getHeader("X-Real-IP");
            if (log.isDebugEnabled()) {
                log.info("[IP X-Real-IP]={}", ip);
            }
        }
        if (StringUtils.isEmpty(ip) || "unknown".equalsIgnoreCase(ip)) {
        ...
        return StringUtils.split(Objects.toString(ip), ",")[0];
    }
     ...
     * @author wangdong
     * @date 2020/7/31 1:54 下午
     **/
    public static boolean isLocalAddr(String ip) {

        return Objects.equals(ip, "127.0.0.1") || Objects.equals(ip, "0:0:0:0:0:0:0:1");
    }
    ...
    private static boolean isIPv4MappedAddress(byte[] paramArrayOfByte) {
        ...
        return (paramArrayOfByte[0] == 0) && (paramArrayOfByte[1] == 0) && (paramArrayOfByte[2] == 0)
                && (paramArrayOfByte[3] == 0) && (paramArrayOfByte[4] == 0)
                && (paramArrayOfByte[5] == 0) && (paramArrayOfByte[6] == 0)
                && (paramArrayOfByte[7] == 0) && (paramArrayOfByte[8] == 0)
                && (paramArrayOfByte[9] == 0) && (paramArrayOfByte[10] == -1)
